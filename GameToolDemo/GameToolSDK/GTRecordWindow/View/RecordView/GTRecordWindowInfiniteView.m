...
@interface GTRecordWindowInfiniteView ()
...
@implementation GTRecordWindowInfiniteView
...
- (void)setUp {
    ...
    [self.durationLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.startButton.mas_right).offset(10 * WIDTH_RATIO);
    ...
    [self.waitTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.startButton.mas_right).offset(10 * WIDTH_RATIO);
        make.bottom.equalTo(self.mas_bottom).offset(-8 * WIDTH_RATIO);
        make.width.mas_equalTo(25 * WIDTH_RATIO);
        make.height.mas_equalTo(14 * WIDTH_RATIO);
    }];
...
- (void)startWaitTimer {
    ...
    //    self.waitTimer = [NSTimer scheduledTimerWithTimeInterval:0.001 target:self selector:@selector(updateWaitTime:) userInfo:@(self.waitTime) repeats:YES];
...
- (void)updateWaitTime:(CADisplayLink *)displayLink {
    ...
    //    NSLog(@"===========%lf========%f", displayLink.duration, self.waitTime);
    //    NSLog(@"%@", displayLink);
    if (self.waitTime < 0) {
        self.waitTime = 0;
    ...
    switch (self.schemeModel.startMethod) {
        ...
        case ClickerWindowStartMethodCountdown: {       //定时回放
            if ([GTRecordManager shareInstance].isPlayback) { //继续或暂停
                ...
                [GTRecordWindowAnimatiion recordWindowFutureInfiniteToFutureCountdownAnimationWithCompletion:^{
...
- (UILabel *)waitTimeDetailLabel {
    if (!_waitTimeDetailLabel) {
        ...
        _waitTimeDetailLabel.text = @"00:00:000";
