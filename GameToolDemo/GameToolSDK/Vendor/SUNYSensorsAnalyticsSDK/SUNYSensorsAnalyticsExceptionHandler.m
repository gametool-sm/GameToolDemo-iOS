//
//  SUNYSensorsAnalyticsExceptionHandler.m
...
@interface SUNYSensorsAnalyticsExceptionHandler ()
...
@implementation SUNYSensorsAnalyticsExceptionHandler

+ (instancetype)sharedHandler {
    static SUNYSensorsAnalyticsExceptionHandler *gSharedHandler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        gSharedHandler = [[SUNYSensorsAnalyticsExceptionHandler alloc] init];
    });
...
- (void)setupHandlers {
    ...
    NSSetUncaughtExceptionHandler(&SUNYSAHandleException);
    
    struct sigaction action;
    sigemptyset(&action.sa_mask);
    action.sa_flags = SA_SIGINFO;
    action.sa_sigaction = &SUNYSASignalHandler;
    ...
    for (int i = 0; i < sizeof(signals) / sizeof(int); i++) {
        ...
        if (err == 0) {
            ...
            SUNYSALog(@"Errored while trying to set up sigaction for signal %d", signals[i]);
        }
    }
}
...
void SUNYSASignalHandler(int crashSignal, struct __siginfo *info, void *context) {
    SUNYSensorsAnalyticsExceptionHandler *handler = [SUNYSensorsAnalyticsExceptionHandler sharedHandler];
...
void SUNYSAHandleException(NSException *exception) {
    SUNYSensorsAnalyticsExceptionHandler *handler = [SUNYSensorsAnalyticsExceptionHandler sharedHandler];
...
- (void) sa_handleUncaughtException:(NSException *)exception {
    ...
    @try {
        for (SUNYSensorsAnalyticsSDK *instance in self.sensorsAnalyticsSDKInstances) {
            NSMutableDictionary *properties = [[NSMutableDictionary alloc] init];
            @try {
                if ([exception callStackSymbols]) {
#if defined(SENSORS_ANALYTICS_CRASH_SLIDEADDRESS)
                    long slide_address = [SUNYSensorsAnalyticsExceptionHandler sa_computeImageSlide];
        ...
        SUNYSALog(@"Encountered an uncaught exception. All SensorsAnalytics instances were archived.");
    } @catch(NSException *exception) {
        SUNYSAError(@"%@ error: %@", self, exception);
    }
