 ...
 *
 * Debug模式有三种选项:
 *   SUNYSensorsAnalyticsDebugOff - 关闭 DEBUG 模式
 *   SUNYSensorsAnalyticsDebugOnly - 打开 DEBUG 模式，但该模式下发送的数据仅用于调试，不进行数据导入
 *   SUNYSensorsAnalyticsDebugAndTrack - 打开 DEBUG 模式，并将数据导入到 SensorsAnalytics 中
 */
typedef NS_ENUM(NSInteger, SUNYSensorsAnalyticsDebugMode) {
    SUNYSensorsAnalyticsDebugOff,
    SUNYSensorsAnalyticsDebugOnly,
    SUNYSensorsAnalyticsDebugAndTrack,
};
 ...
 *
 * @discussion
 *   SUNYSensorsAnalyticsNetworkTypeNONE - NULL
 *   SUNYSensorsAnalyticsNetworkType2G - 2G
 *   SUNYSensorsAnalyticsNetworkType3G - 3G
 *   SUNYSensorsAnalyticsNetworkType4G - 4G
 *   SUNYSensorsAnalyticsNetworkTypeWIFI - WIFI
 *   SUNYSensorsAnalyticsNetworkTypeALL - ALL
 */
typedef NS_OPTIONS(NSInteger, SUNYSensorsAnalyticsNetworkType) {
    SUNYSensorsAnalyticsNetworkTypeNONE      = 0,
    SUNYSensorsAnalyticsNetworkType2G       = 1 << 0,
    SUNYSensorsAnalyticsNetworkType3G       = 1 << 1,
    SUNYSensorsAnalyticsNetworkType4G       = 1 << 2,
    SUNYSensorsAnalyticsNetworkTypeWIFI     = 1 << 3,
    SUNYSensorsAnalyticsNetworkTypeALL      = 0xFF,
};

/**
 * @class
 * SUNYSensorsAnalyticsSDK 类
 *
 * @abstract
 * 在 SDK 中嵌入 SensorsAnalytics 的 SDK 并进行使用的主要 API
 *
 * @discussion
 * 使用SUNYSensorsAnalyticsSDK类来跟踪用户行为，并且把数据发给所指定的SensorsAnalytics的服务。
 */
@interface SUNYSensorsAnalyticsSDK : NSObject
...
/**
 * @abstract
 * 根据传入的配置，初始化并返回一个 SUNYSensorsAnalyticsSDK 的单例
 *
 ...
 * @return 返回的单例
 */
+ (SUNYSensorsAnalyticsSDK *)sharedInstanceWithServerURL:(nullable NSString *)serverURL
 ...
 * @return 返回的单例
 */
+ (SUNYSensorsAnalyticsSDK *)sharedInstanceWithServerURL:(nonnull NSString *)serverURL
 ...
 *
 * @param networkType SUNYSensorsAnalyticsNetworkType
 */
 ...
 *
 * @discussion
 * 根据需要决定是否开启 SDK log , SUNYSensorsAnalyticsDebugOff 模式默认关闭 log
 * SUNYSensorsAnalyticsDebugOnly  SUNYSensorsAnalyticsDebugAndTrack 模式默认开启log
 *
 ...
 *
 * @discussion
 * 注意：清除 keychain 中 kSUNYSAService 名下的数据，包括 distinct_id 和 AppInstall 标记。
