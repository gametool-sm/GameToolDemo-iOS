...
@implementation SUNYSALogger
+ (void)initialize {
    __enableLog__ = NO;
    __logQueue__ = dispatch_queue_create("com.sensorsdata.analytics.mini.SUNYSALogger.queue", DISPATCH_QUEUE_SERIAL);
}
...
- (void)log:(BOOL)asynchronous
   ...
   function:(const char *)function
       line:(NSUInteger)line {
    @try{
        NSString *logMessage = [[NSString alloc]initWithFormat:@"[SUNYSALog][%@]  %s [line %lu]    %s %@",[self descriptionForLevel:level],function,(unsigned long)line,[@"" UTF8String],message];
...
-(NSString *)descriptionForLevel:(SUNYSALoggerLevel)level {
    NSString *desc = nil;
    switch (level) {
        case SUNYSALoggerLevelInfo:
            desc = @"INFO";
            break;
        case SUNYSALoggerLevelWarning:
            desc = @"WARN";
            break;
        case SUNYSALoggerLevelError:
            desc = @"ERROR";
            break;
        default:
            desc = @"UNKNOW";
