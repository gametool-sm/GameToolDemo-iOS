...
- (NSData *)JSONSerializeObject:(id)obj {
    ...
    @catch (NSException *exception) {
        SUNYSAError(@"%@ exception encoding api data: %@", self, exception);
    }
    if (error) {
        SUNYSAError(@"%@ error encoding api data: %@", self, error);
    }
    return data;
}
...
- (id)JSONSerializableObjectForObject:(id)obj {
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        for (id key in obj) {
            ...
            if (![key isKindOfClass:[NSString class]]) {
                ...
                SUNYSAError(@"%@ warning: property keys should be strings. got: %@. coercing to: %@", self, [key class], stringKey);
            } else {
    ...
    SUNYSAError(@"%@ warning: property values should be valid json types. got: %@. coercing to: %@", self, [obj class], s);
    return s;
}
