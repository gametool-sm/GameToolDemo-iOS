...
NSString * const kSUNYSAService = @"com.sensorsdata.analytics.mini.SUNYSAKeyChainItemWrapper.udid";
NSString * const kSUNYSAUdidAccount = @"com.sensorsdata.analytics.mini.SUNYSAKeyChainItemWrapper.udid";

@implementation SUNYSAKeyChainItemWrapper
+ (NSString *)saUdid {
    NSDictionary *result = [self fetchPasswordWithAccount:kSUNYSAUdidAccount service:kSUNYSAService];
    NSString *sa_udid =  [result objectForKey:(__bridge id)kSecValueData];
    return sa_udid;
}

+ (NSString *)saveUdid:(NSString *)udid {
    BOOL sucess = [self saveOrUpdatePassword:udid account:kSUNYSAUdidAccount service:kSUNYSAService];
...
+ (BOOL)saveOrUpdatePassword:(NSString *)password account:(NSString *)account service:(NSString *)service accessGroup:(NSString *)accessGroup {
    @try{
        ...
        if (isSUNYStringParamValid(account)) {
            [query setObject:account forKey:(__bridge id) kSecAttrAccount];
        }
        if (isSUNYStringParamValid(service)) {
            [query setObject:service forKey:(__bridge id) kSecAttrService];
        }
#if !TARGET_IPHONE_SIMULATOR
        if (isSUNYStringParamValid(accessGroup)) {
        ...
        if (status == errSecSuccess) {
            ...
            SUNYSALog(@"SecItemUpdate result = %d",result);
            ...
            SUNYSALog(@"SecItemAdd result = %d",status);
        }
...
BOOL isSUNYStringParamValid(id  parameter){
    BOOL result = NO;
