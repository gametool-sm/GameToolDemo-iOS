...
@interface GTSpeedUpTipViewController ()
...
@implementation GTSpeedUpTipViewController
...
- (void)startClick {
    ...
    model.isUp = YES;
    [GTSDKUtils saveSpeedUpOfSpeed:@[model]];
    //记录已看过tip页面
    [GTSDKUtils saveReadSpeedUpTutorials];
    
        
    //工具箱元素点击埋点
    [[GTDataConfig sharedInstance] uploadSensorDataWithEvent:GTSensorEventToolBoxElementClick andProperties:@{@"tool_name" : @"加速器", @"plan_id" : [NSNumber numberWithInt:-1],  @"toolbox_click_type" : [NSNumber numberWithInt:1]} shouldFlush:YES];
...
- (GTIntroduceView *)introduceView {
    if (!_introduceView) {
        _introduceView = [[GTIntroduceView alloc] initWithDescText:@"加速器是一款合规的软件变速工具，在不影响软件性能的前提下可加快软件运行速度节约操作时间。"  bundleName:@"accelerator_specification"];
    }
