//
//  GTRecordSetViewController.m
//  GTSDK
//
//  Created by shangmi on 2023/10/24.
//
...
@interface GTRecordSetViewController ()
...
@implementation GTRecordSetViewController
...
- (void)revealClick:(GTSwitch *)sender {
    ...
    if (!self.revealSwitchButton.on) {
        ...
        [GTRecordWindowManager shareInstance].pointSetModel.pointShowType = ClickerWindowPointShowNo;
        ...
        if (pointOpera == 0) {
            //显示运行按钮的功能
            [GTRecordWindowManager shareInstance].pointSetModel.pointShowType = ClickerWindowPointShowExecute;
        }else {
            //显示运行按钮的功能
            [GTRecordWindowManager shareInstance].pointSetModel.pointShowType = ClickerWindowPointShowAll;
        }
        [UIView animateWithDuration:0.12 animations:^{
            [self.revealView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.mas_equalTo(70 * WIDTH_RATIO);
...
- (void)PointClick:(UIButton *)button{
    ...
    switch (self.selectedButtonIndex) {
        case 0:{
            ...
            //将触点图片设置为标准
            [GTRecordWindowManager shareInstance].pointSetModel.pointSize = ClickerWindowPointSizeOfMedium;
        }
            break;
        case 1:{
            ...
            //将触点图片设置为小的
            [[GTRecordWindowManager shareInstance].pointSetModel setPointSize:ClickerWindowPointSizeOfSmall];
        }
            break;
        case 2:{
            self.bigPointButton.selected = YES;
            ...
            [GTRecordWindowManager shareInstance].pointSetModel.pointSize = ClickerWindowPointSizeOfLarge;
        }
            break;
        default:
            break;
