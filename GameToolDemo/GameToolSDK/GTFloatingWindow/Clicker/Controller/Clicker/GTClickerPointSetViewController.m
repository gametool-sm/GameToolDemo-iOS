//
//  GTClickerPointSetViewController.m
//  GTSDK
//
//  Created by shangminet on 2023/8/22.
//
...
@interface GTClickerPointSetViewController ()<UITableViewDelegate, UITableViewDataSource,GTClickerPointSetTableViewCellDelegate,UITableViewDropDelegate,UITableViewDragDelegate,UITextFieldDelegate>
...
@implementation GTClickerPointSetViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tapSetNotification:) name:GTSDKClickerWindowTapSetNotification object:nil];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillShow:) name:UIKeyboardWillShowNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillHide:) name:UIKeyboardWillHideNotification object:nil];
    
    [[GTClickerManager shareInstance] addObserver:self forKeyPath:@"isRun" options:NSKeyValueObservingOptionNew context:nil];
    //监听悬浮窗视图样式
    [[GTClickerWindowManager shareInstance] addObserver:self forKeyPath:@"clickerWindowState" options:NSKeyValueObservingOptionNew context:nil];
...
- (void)QuitSchemeNotification:(NSNotification *)noti{
    ...
    //AutoClickerRunDurationEvent 连点器使用时长 CP&Sensor 退出方案
    //postNotificationName:GTSDKClickerWindowQuitSchemeNotification
    [SMEventSensor finishReport:AutoClickerRunDurationEvent];
    //AutoClickerStartDurationEvent 连点器启用时长 Sensor Onlu 退出方案
    [SMEventSensor finishReport:AutoClickerStartDurationEvent];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqual:@"isRun"] || [keyPath isEqual:@"clickerWindowState"]) {
        if(([GTClickerManager shareInstance].isRun ||
           ...
           [GTClickerWindowManager shareInstance].clickerWindowState == ClickerWindowStateFutureDark )&&
            ...
            [self.clickerWindowStartView updateDataWithType:SchemeActionPageTypeClickerPlaying];
...
-(void)PauseSetNotification:(NSNotification *)noti {
    ...
    //AutoClickerRunDurationEvent 连点器使用时长 CP&Sensor 结束方案
    //postNotificationName:GTSDKClickerWindowPauseNotification
    [SMEventSensor finishReport:AutoClickerRunDurationEvent];
...
-(void)loadData{
    ...
    for (GTClickerActionModel *actionModel in  [GTClickerWindowManager shareInstance].compareArray) {
...
- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath{
    ...
    [GTClickerWindowManager shareInstance].schemeModel.actionArray = [NSMutableArray arrayWithArray:copyArray];
    ...
    [GTClickerWindowManager shareInstance].compareArray = [NSMutableArray arrayWithArray:copyArray2] ;
...
- (void)deleteButtonClickedInCell:(NSIndexPath *)indexPath {
    if(self.pointDataArray.count == 1){
        ...
        dialogView = [[GTDialogView alloc] initWithStyle:DialogViewStyleDefault
            ...
            [[GTDialogWindowManager shareInstance] dialogWindowHide];
            //删除此触点model
            [[GTClickerWindowManager shareInstance].schemeModel.actionArray removeObjectAtIndex:indexPath.row];
            [[GTClickerWindowManager shareInstance].compareArray removeObjectAtIndex:indexPath.row];
            //删除此触点window
            [[GTClickerWindowManager shareInstance].pointWindowArray removeObjectAtIndex:indexPath.row];
...
- (void)cellDidFinishEditingWithData:(GTClickerActionModel *)data indexPath:(NSIndexPath *)indexPath {
    ...
    [GTClickerWindowManager shareInstance].schemeModel.actionArray = [NSMutableArray arrayWithArray:copyArray2];
...
-(void)setClicker:(id)sender{
    //工具箱元素点击埋点
    [[GTDataConfig sharedInstance] uploadSensorDataWithEvent:GTSensorEventToolBoxElementClick andProperties:@{@"tool_name" : @"连点器", @"plan_id" : [NSNumber numberWithInt:-1],  @"toolbox_click_type" : [NSNumber numberWithInt:10]} shouldFlush:YES];
...
-(void)quitClick:(id)sender{
    if ([[GTClickerWindowManager shareInstance].schemeJsonString isEqualToString:@""]) {
        ...
        //工具箱元素点击埋点
        [[GTDataConfig sharedInstance] uploadSensorDataWithEvent:GTSensorEventToolBoxElementClick andProperties:@{@"tool_name" : @"连点器", @"plan_id" : [NSNumber numberWithInt:-2],  @"toolbox_click_type" : [NSNumber numberWithInt:7]} shouldFlush:YES];
    }
    ...
    if ([self.saveButton.backgroundColor isEqual:[UIColor themeColor]]) {
        ...
        dialogView = [[GTDialogView alloc] initWithStyle:DialogViewStyleDefault
            ...
            changeOnce = NO;
            [[GTDialogWindowManager shareInstance] dialogWindowHide];
            //AutoClickerStartDurationEvent 连点器启动时长 退出方案
            [SMEventSensor finishReport:AutoClickerStartDurationEvent];
        }];
        [[GTDialogWindowManager shareInstance].dialogVC.view addSubview:dialogView];
        [[GTDialogWindowManager shareInstance] dialogWindowShow];
...
-(void)saveClick:(id)sender{
    if ([[GTClickerWindowManager shareInstance].schemeJsonString isEqualToString:@""]) {
        ...
        //工具箱元素点击埋点
        [[GTDataConfig sharedInstance] uploadSensorDataWithEvent:GTSensorEventToolBoxElementClick andProperties:@{@"tool_name" : @"连点器", @"plan_id" : [NSNumber numberWithInt:-2],  @"toolbox_click_type" : [NSNumber numberWithInt:8]} shouldFlush:YES];
    }
       ...
       if(self.planView.planNameTextField.text.length == 0){
            ...
            dialogView = [[GTDialogView alloc] initWithStyle:DialogViewStyleDefault
                ...
                if(self.DataArray.count > self.row){
                    ...
                    [dataArray replaceObjectAtIndex:self.row withObject:[GTClickerWindowManager shareInstance].schemeModel];
                    self.DataArray = dataArray;
                    [NSData saveSchemeList:self.DataArray];
                    [GTClickerWindowManager shareInstance].schemeJsonString = [[GTClickerWindowManager shareInstance].schemeModel modelToJSONString];
                    
                    self.planView.startTimeCompare = self.planView.startTime;
                    self.planView.oldSelected = self.planView.selectedButtonIndex;
                    self.planView.cycleIndex = [GTClickerWindowManager shareInstance].schemeModel.cycleIndex;
                    self.planView.cycleInterval = [GTClickerWindowManager shareInstance].schemeModel.cycleInterval;
                }else{
                    //创建
                    NSMutableArray<GTClickerSchemeModel *> *dataArray = self.DataArray;
                    [dataArray addObject:[GTClickerWindowManager shareInstance].schemeModel];
                    self.DataArray = dataArray;
                    [NSData saveSchemeList:self.DataArray];
                    [GTClickerWindowManager shareInstance].schemeJsonString = [[GTClickerWindowManager shareInstance].schemeModel modelToJSONString];
                ...
                self.compareDataArray = [NSMutableArray arrayWithArray:copyArray];
                [GTClickerWindowManager shareInstance].compareArray = [copyArray mutableCopy];
                [self.pointSetTableView reloadData];
                [[GTDialogWindowManager shareInstance] dialogWindowHide];
            }];
            [[GTDialogWindowManager shareInstance].dialogVC.view addSubview:dialogView];
...
-(GTClickerPointSetPlanView *)planView{
    if(!_planView){
        _planView = [GTClickerPointSetPlanView new];
    }
    return _planView;
}
-(UIView *)lineView{
