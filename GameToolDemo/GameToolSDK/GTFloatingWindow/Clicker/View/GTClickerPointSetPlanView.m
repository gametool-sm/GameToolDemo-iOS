//
//  GTClickerPointSetPlanView.m
//  GTSDK
//
//  Created by shangminet on 2023/8/23.
//

#import "GTClickerPointSetPlanView.h"
//#import "GTClickerWindowManager.h"
//#import "UIButton+Extent.h"
//#import "GTFloatingWindowManager.h"

@interface GTClickerPointSetPlanView () <UIScrollViewDelegate>

@end

@implementation GTClickerPointSetPlanView
...
- (void)textFieldDidChange:(UITextField *)textField {
   ...
   self.startTime = [NSString stringWithFormat:@"%@:%@:%@", self.timeHour.text, self.timeMinute.text, self.timeSecond.text] ;
    ...
    //修改值后，传给model
    [GTClickerWindowManager shareInstance].schemeModel.name = self.planNameTextField.text;
    
    if(self.fincycleIndex){
        //有限循环次数
        [GTClickerWindowManager shareInstance].schemeModel.cycleIndex = [self.loopNumberTextField.text intValue];
    }else{
        [GTClickerWindowManager shareInstance].schemeModel.cycleIndex = 0;
    }
    ...
    //循环间隔
    [GTClickerWindowManager shareInstance].schemeModel.cycleInterval = [self.loopIntervalTextField.text intValue];
    [GTClickerWindowManager shareInstance].schemeModel.startTime = self.startTime;
...
-(void)limitedNumberClicked{
    if(!self.isLimitedNumberSelected){
        ...
        if([self.loopNumberTextField.text intValue] == 0){
            [GTClickerWindowManager shareInstance].schemeModel.cycleIndex = 1;
